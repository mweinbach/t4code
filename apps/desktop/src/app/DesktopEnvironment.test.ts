...
describe("DesktopEnvironment", () => {
  it.effect("derives state paths and development identity inside Effect", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.displayName, "T4 Code (Dev)");
      assert.equal(environment.userDataDirName, "t4code-dev");
      ...
      assert.equal(environment.appUserModelId, "com.mweinbach.t4code.dev");
  ...
  it.effect("stores production state under userdata in an explicit home", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.appUserModelId, "com.mweinbach.t4code");
      assert.equal(environment.stateDir, "/tmp/t3/userdata");
      assert.equal(environment.logDir, "/tmp/t3/userdata/logs");
      assert.equal(environment.browserArtifactsDir, "/tmp/t3/userdata/browser-artifacts");
      assert.equal(environment.serverSettingsPath, "/tmp/t3/userdata/settings.json");
      assert.equal(environment.userDataDirName, "t4code");
      assert.equal(environment.legacyUserDataDirName, "T4 Code (Alpha)");
    }),
  );
  ...
  it.effect("uses a configured app user model id override", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          T3CODE_DESKTOP_APP_USER_MODEL_ID: " com.mweinbach.t4code.dev.local ",
          VITE_DEV_SERVER_URL: "http://localhost:5173",
        },
      );

      assert.equal(environment.appUserModelId, "com.mweinbach.t4code.dev.local");
    }),
  );
