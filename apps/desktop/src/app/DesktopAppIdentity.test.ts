          ...
          FileSystem.layerNoop({
            exists: (path) =>
              input.legacyPathProbeError
                ...
                : Effect.succeed(
                    (input.legacyPathExists === true && path.endsWith("T4 Code (Alpha)")) ||
                      (input.upstreamPathExists === true && path.endsWith("T3 Code (Alpha)")),
                  ),
            readFileString: () =>
...
describe("DesktopAppIdentity", () => {
  it.effect("does not reuse the installed T3 Code browser data", () =>
    withIdentity(
      Effect.gen(function* () {
        ...
        assert.equal(
          ...
          "/Users/alice/Library/Application Support/t4code",
        );
      }),
      { upstreamPathExists: true },
    ),
  );
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["T4 Code"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "T4 Code");
