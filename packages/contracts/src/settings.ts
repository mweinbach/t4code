...
export const OpenGrokSettings = makeProviderSettingsSchema(
  {
    enabled: Schema.Boolean.pipe(
      Schema.withDecodingDefault(Effect.succeed(true)),
      Schema.annotateKey({ providerSettingsForm: { hidden: true } }),
    ),
    binaryPath: makeBinaryPathSetting("open-grok").pipe(
      Schema.annotateKey({
        title: "Binary path",
        description: "Path to the OpenGrok CLI binary on this environment.",
        providerSettingsForm: { placeholder: "open-grok", clearWhenEmpty: "omit" },
      }),
    ),
    customModels: Schema.Array(Schema.String).pipe(
      Schema.withDecodingDefault(Effect.succeed([])),
...
export type OpenGrokSettings = typeof OpenGrokSettings.Type;
...
export const ServerSettings = Schema.Struct({
  ...
  providers: Schema.Struct({
    ...
    "open-grok": OpenGrokSettings.pipe(Schema.withDecodingDefault(Effect.succeed({}))),
...
const OpenGrokSettingsPatch = Schema.Struct({
  enabled: Schema.optionalKey(Schema.Boolean),
  binaryPath: Schema.optionalKey(TrimmedString),
  customModels: Schema.optionalKey(Schema.Array(Schema.String)),
});
...
export const ServerSettingsPatch = Schema.Struct({
  ...
  providers: Schema.optionalKey(
    Schema.Struct({
      ...
      "open-grok": Schema.optionalKey(OpenGrokSettingsPatch),
